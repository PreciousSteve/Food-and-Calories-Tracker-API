...
def update_food(food_id:int, food:Food, session=Depends(get_db), current_user=Depends(get_current_active_user)):
    ...
    if existing_food.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to update this food data")
...
def remove_food(food_id:int, session=Depends(get_db), current_user=Depends(get_current_active_user)):
    ...
    if existing_food is None:
        raise HTTPException(status_code=404, detail="Food data not found")
    if existing_food.user_id != current_user.id:  
        raise HTTPException(status_code=403, detail="Not authorized to delete this food data")
