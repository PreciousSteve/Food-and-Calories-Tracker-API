...
@router.post(
    "/users/me/calorie-goal", description="Set or update the user's daily calorie goal."
)
def set_calorie_goal(
    ...
    if existing_goal:
        ...
        create_calorie_goal(
            session=session, goal=goal, user_id=current_user.id
        )
...
@router.get(
    "/users/me/calorie-goal",
    description="Retrieve the user's current daily calorie goal.",
)
def get_goal(
...
@router.get(
    "/users/me/calorie-goal-progress",
    description="Check the user's calorie goal, comparing calories consumed with the goal.",
)
def get_caloric_goal_progress(
    session: Session = Depends(get_db), current_user=Depends(get_current_active_user)
):
