...
def get_reminders(session:Session, user_id:int):
    return session.query(reminder_model.Reminder).filter(reminder_model.Reminder.user_id == user_id).all()

def get_reminder_by_id(session:Session, reminder_id:int):
    return session.query(reminder_model.Reminder).filter(reminder_model.Reminder.id == reminder_id).first()
