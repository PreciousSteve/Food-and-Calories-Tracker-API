...
def create_food(session: Session, food: Food, user_id: int):
    profile = (
        session.query(profile_model.Profile)
...
def get_food(session: Session, user_id: int):
    return (
        session.query(food_model.Food).filter(food_model.Food.user_id == user_id).all()
    )
...
def get_food_by_name(session: Session, name: str, user_id: int):
    return (
        session.query(food_model.Food)
        .filter(food_model.Food.name == name, food_model.Food.user_id == user_id)
        .first()
    )
...
def edit_food(session: Session, food_id: int, food: Food):
    add_edit = (
        session.query(food_model.Food).filter(food_model.Food.id == food_id).first()
    )
    if not add_edit:
        return None
...
def get_user_total_calories(session: Session, user_id: int):
    total_calories = (
        session.query(func.sum(food_model.Food.kcal_per_serving))
        .filter(food_model.Food.user_id == user_id)
        .scalar()
    )
